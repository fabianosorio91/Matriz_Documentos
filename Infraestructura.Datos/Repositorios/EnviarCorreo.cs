...
namespace Infraestructura.Datos.Repositorios
{
    public class EnviarCorreo
    {
        ...
        /// para correo cuando es un update
        /// 


        public async Task SendEmailModificado(List<OrdenCompra> oLista)
        {
            try
            {
                var correo = "";
                ...
                using (var conexion = new SqlConnection(cadenaSQL))
                {
                    ...
                    using (var dr = await cmd.ExecuteReaderAsync())
                    {
                        while (await dr.ReadAsync())
                        {
                            ...
                            if (parametros.TryGetValue("asuntoM", out string resasunto))
                            {
                                asunto = resasunto;
                            }

                            if (parametros.TryGetValue("cuerpoM", out string rescuerpo))
                            {
                                cuerpo = rescuerpo;
                            }
                        }
